...
export const mission = {
  next: async (spaceAgency: string | null): Promise<NextType> => {
    const result = (
      await requests.getMission(
        !spaceAgency ? "launch/upcoming/?limit=1&offset=1%22&mode=detailed" : "launch/upcoming/?search=" + `${spaceAgency}&mode=detailed`
      )
    ).results;
    if (result.length) {
      log(result[0]);
  ...
  limit: async (spaceAgency?: string, limit?: number): Promise<NextType[]> => {
    return (
      await requests.getMission(
        !spaceAgency ? "launch/upcoming/?limit=" + `${limit}&mode=detailed` : "launch/upcoming/?search=" + `${spaceAgency}&mode=detailed`
      )
    ).results;
  },
};